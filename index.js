...
server.tool(
  ...
  async (args) => {
    try {
      const { url, method = "GET", headers = {}, params = {}, data = {}, timeout = 30000 } = args;
      // 전체 매개변수 객체 로깅
      console.error(`API Caller MCP Server: Received parameters:`, JSON.stringify(params, null, 2));
      console.error(`API Caller MCP Server: Direct parameters:`, { url, method, headers, params, data, timeout });
      ...
      const queryParams = (params && (params.params || params.queryParams)) || {};
      ...
      if (!['GET', 'HEAD', 'OPTIONS'].includes(methodToUse.toUpperCase())) {
